// resources/js/pages/admin/Salas.jsx
...
import {
    ...
    AlertTriangle, CheckCircle, XCircle, MapPin, Users, DoorOpen
} from 'lucide-react';
...
export default function Salas() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editando) {
                await api.put(`/salas/${editando}`, payload);
            } else {
    ...
    return (
        <Layout>
            <div className="space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <DoorOpen size={24} className={isDark ? 'text-indigo-400' : 'text-indigo-600'} />
                        <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>Salas</h2>
                    </div>
                    <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                        Gestiona las salas disponibles para eventos
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className={`rounded-2xl shadow-lg overflow-hidden ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'}`}>
                        ...
                        <div className="p-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    <label className={labelClass}><DoorOpen size={14} />Nombre *</label>
                                    <input type="text" value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })}
                                        className={inputClass} placeholder="Ej: Sala de Juntas, Auditorio" required />
                                </div>
                                <div>
                                    <label className={labelClass}><MapPin size={14} />Ubicación</label>
                                    <input type="text" value={form.ubicacion} onChange={e => setForm({ ...form, ubicacion: e.target.value })}
                                        className={inputClass} placeholder="Ej: Piso 2, Bloque A" />
                                </div>
                                <div>
                                    <label className={labelClass}><Users size={14} />Capacidad</label>
                                    <input type="number" value={form.capacidad} onChange={e => setForm({ ...form, capacidad: e.target.value })}
                                        className={inputClass} placeholder="Número de personas" min="1" />
                                </div>
                    ...
                    <div className={`lg:col-span-2 rounded-2xl shadow-lg overflow-hidden ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-100'}`}>
                        <div className={`px-6 py-4 border-b ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
                            <div className="flex items-center gap-2">
                                <DoorOpen size={18} className={isDark ? 'text-indigo-400' : 'text-indigo-600'} />
                                <h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-800'}`}>Listado de Salas</h3>
                                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${isDark ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-600'}`}>
                                    {salas.length} registros
                                </span>
                            </div>
                        </div>
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className={`sticky top-0 ${isDark ? 'bg-gray-800' : 'bg-white'} shadow-sm`}>
                                            <tr className={isDark ? 'bg-gray-900/50 border-b border-gray-700' : 'bg-gray-50 border-b border-gray-200'}>
                                                {['Sala', 'Ubicación', 'Capacidad', 'Estado', 'Acciones'].map(h => (
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                            {salas.map(s => (
                                                <tr key={s.id} className={`transition ${isDark ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}`}>
                                                    <td className={`px-6 py-3 font-medium ${isDark ? 'text-white' : 'text-gray-800'}`}>{s.nombre}</td>
                                                    <td className={`px-6 py-3 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{s.ubicacion || '-'}</td>
                                                    <td className={`px-6 py-3 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                                        {s.capacidad ? <span className="flex items-center gap-1"><Users size={13} />{s.capacidad} personas</span> : '-'}
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-3">
                                                        <div className="flex items-center gap-2">
                                                            <button onClick={() => handleEditar(s)} className={`p-1.5 rounded-lg transition ${isDark ? 'hover:bg-gray-700 text-gray-400 hover:text-indigo-400' : 'hover:bg-gray-100 text-gray-500 hover:text-indigo-600'}`} title="Editar"><Edit size={16} /></button>
                                                            <button onClick={() => openModal(s.id, s.nombre)} className={`p-1.5 rounded-lg transition ${isDark ? 'hover:bg-gray-700 text-gray-400 hover:text-red-400' : 'hover:bg-gray-100 text-gray-500 hover:text-red-600'}`} title="Desactivar"><Trash2 size={16} /></button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                                            {salas.length === 0 && (
                                                <tr>
                                                    <td colSpan="5" className={`px-6 py-12 text-center ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                                                        <DoorOpen size={48} className={`mx-auto mb-3 ${isDark ? 'text-gray-700' : 'text-gray-300'}`} />
                                                        <p>No hay salas registradas</p>
                                                        <p className="text-xs mt-1">Comienza creando una nueva sala</p>
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                                    ...
                                    {salas.map(s => (
                                        <div key={s.id} className={`p-4 ${isDark ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}`}>
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    <h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-800'}`}>{s.nombre}</h3>
                                                    {s.ubicacion && <div className={`flex items-center gap-1 mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}><MapPin size={13} />{s.ubicacion}</div>}
                                                    {s.capacidad && <div className={`flex items-center gap-1 mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}><Users size={13} />{s.capacidad} personas</div>}
                                                ...
                                                <div className="flex gap-1">
                                                    <button onClick={() => handleEditar(s)} className={`p-1.5 rounded-lg transition ${isDark ? 'hover:bg-gray-700 text-gray-400 hover:text-indigo-400' : 'hover:bg-gray-100 text-gray-500 hover:text-indigo-600'}`}><Edit size={16} /></button>
                                                    <button onClick={() => openModal(s.id, s.nombre)} className={`p-1.5 rounded-lg transition ${isDark ? 'hover:bg-gray-700 text-gray-400 hover:text-red-400' : 'hover:bg-gray-100 text-gray-500 hover:text-red-600'}`}><Trash2 size={16} /></button>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
            ...
            <SuccessModal 
                ...
                title="¡Sala Creada!"
                message="La nueva sala ha sido registrada correctamente."
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className={`relative w-full max-w-md rounded-2xl shadow-2xl overflow-hidden ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                        <div className="p-6 text-center">
                            ...
                            <h3 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>Desactivar sala</h3>
                            <p className={`text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                ¿Estás seguro de que deseas desactivar la sala <span className="font-semibold">"{modalConfirm.nombre}"</span>?
                            </p>
